...
// One producer thread, three consumer threads, one queue -> SPMC.
...
module {
  ...
  cir.func @main() {
    ...
    %a = cir.get_global @consumer_a : !cir.ptr<!cir.func<()>>
    cir.call @spawn(%a) : (!cir.ptr<!cir.func<()>>) -> ()
    %b = cir.get_global @consumer_b : !cir.ptr<!cir.func<()>>
    cir.call @spawn(%b) : (!cir.ptr<!cir.func<()>>) -> ()
    %c = cir.get_global @consumer_c : !cir.ptr<!cir.func<()>>
...
// CHECK-NEXT: consumers: 3
// CHECK-NEXT: classification: SPMC
